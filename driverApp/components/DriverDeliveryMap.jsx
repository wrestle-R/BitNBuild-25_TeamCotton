...
const DriverDeliveryMap = ({ delivery, driverLocation }) => {
  ...
  console.log('DriverDeliveryMap - Received delivery:', delivery ? 'Yes' : 'No');
  console.log('DriverDeliveryMap - Driver location:', driverLocation ? 'Yes' : 'No');

  // TSP Solver - Nearest Neighbor Heuristic (adapted for React Native)
  ...
  useEffect(() => {
    ...
    console.log('Raw vendor address:', JSON.stringify(delivery.vendorId.address, null, 2));
    console.log('Raw first customer:', JSON.stringify(delivery.customers?.[0]?.customerId?.address, null, 2));
    
    // Extract vendor location - handle both {lat, lng} and [lng, lat] formats
    let vendorCoordinates;
    if (delivery.vendorId.address.coordinates) {
      const coords = delivery.vendorId.address.coordinates;
      if (coords.lat !== undefined && coords.lng !== undefined) {
        // {lat, lng} format from schema
        vendorCoordinates = [coords.lng, coords.lat];
        console.log('✅ Extracted vendor coordinates from {lat, lng}:', vendorCoordinates);
      } else if (Array.isArray(coords) && coords.length === 2) {
        // [lng, lat] GeoJSON format
        vendorCoordinates = coords;
        console.log('✅ Using GeoJSON vendor coordinates:', vendorCoordinates);
      }
    }
    ...
    // Extract customer locations - handle both formats
    ...
    console.log('Customer locations:', customers.length);
    
    if (customers.length === 0) {
      console.log('⚠️ No valid customer locations found');
      // Still show vendor on map even if no customers
      setRouteInfo({
        totalTime: 0,
        totalDistance: '0',
        customers: 0,
      ...
      // Set map region to vendor location
      setTimeout(() => {
        if (mapRef.current) {
  ...
  const formatAddress = (addressObj) => {
    ...
    // If it has a pre-formatted string, use it
    if (addressObj.formatted) return addressObj.formatted;
    ...
    return (
      <View style={{ 
        ...
        <Text style={{ 
          ...
          Vendor location or customer coordinates are missing
        </Text>
      </View>
    );
  }
